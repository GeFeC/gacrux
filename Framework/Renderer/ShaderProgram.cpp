...
namespace program = gx::renderer::shader_program;
...
auto program::link_shaders() -> void{
  ...
  throw ShaderException("Failed to link shader with error:\n" + std::string{ msg.data() } );
}
...
auto program::set_uniform(const std::string& uniform_name, i32 value) noexcept -> void{
  glUniform1i(
    glGetUniformLocation(program_id, uniform_name.c_str()), 
    value
  );
